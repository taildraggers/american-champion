...
Daily aggregator of American Champion aircraft classified listings from
...
- `scraper/barnstormers.py` searches Barnstormers.com for American Champion
...
- Only whole-aircraft-for-sale listings are published. Each ad's title must state a
  model year and match a recognized American Champion model code/name (7GCAA, 8KCAB,
  Super Decathlon, Citabria, Scout, Explorer, etc. - see `_MODEL_CODE_RE`/
  `_MODEL_NAME_RULES` in `scraper/barnstormers.py`); titles that read as parts,
  accessories, services, or raffles are dropped. Every surviving listing's title is
  rewritten to a canonical **`YEAR American Champion MODEL`** form (e.g. `1997
  American Champion Super Decathlon`), regardless of how the original ad was
  worded, so the page reads consistently. A real side effect: ads that never state
  a model year in the title can't be reformatted and are dropped too, even if
  they're genuine aircraft.
- `main.py` runs the scraper, de-duplicates results, and renders them into
  `docs/index.html` titled **"Other American Champion Ads on the Web"**, with
...
4. GitHub will publish the page at `https://taildraggers.github.io/american-champion/`
...
```html
<iframe
  src="https://taildraggers.github.io/american-champion/"
  title="Other American Champion Ads on the Web"
